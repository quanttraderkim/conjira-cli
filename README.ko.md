...
문서를 올릴 때는 단순히 내용을 맞게 넣는 것보다, 읽는 사람이 빠르게 판단할 수 있는 구조로 만드는 것이 중요합니다. 특히 전략 메모, PRD, 서비스기획 문서, 보고용 문서는 아래 원칙을 같이 요청해두면 결과가 훨씬 안정적입니다.

기본 문체는 장황한 `~입니다`, `~합니다`보다 `~함`, `~필요`, `~전제`, `~검토`, `~제안`처럼 짧은 메모형 표현을 권장합니다. 문단은 짧게 유지하고, 결론과 핵심 판단을 앞에 둡니다. `###` 제목은 단순 라벨이 아니라 핵심 메시지나 판단문 역할을 하게 두는 것이 좋습니다.

불릿은 병렬 정보일 때만 쓰고, 한 문장을 억지로 잘게 나누는 용도로는 쓰지 않는 편이 좋습니다. 비교, 요약, 옵션, 리스크, 타깃, R&R처럼 한눈에 봐야 하는 정보는 표로 정리하는 쪽이 더 읽기 좋습니다. 반대로 짧은 문장 두세 줄로 끝날 수 있는 내용은 억지로 표로 만들지 않는 편이 좋습니다.

문서 구조는 보통 `목적 -> 핵심 요약 -> 핵심 판단/근거 -> 리스크/전제 -> 시사점/다음 액션` 순서가 가장 무난합니다. 전략 메모라면 `왜 중요한가`, `핵심 판단`, `근거`, `시사점`이 잘 드러나야 하고, PRD라면 `문제 정의`, `목표`, `범위`, `흐름`, `정책`, `오픈 이슈`가 분명해야 합니다. skill spec은 `한 줄 정의`, `언제 쓰는가`, `입력`, `출력`, `예외`, `평가 기준`처럼 더 건조하게 가는 것이 좋습니다.

추상적인 표현이나 과한 수식어는 줄이는 편이 좋습니다. 예를 들어 `전략적으로 의미 있는 방향`, `본질적으로 중요한 가치`, `다양한 측면에서 유의미한 효과` 같은 문구는 되도록 피하고, 대신 `후속 연락 누락이 매출 누락으로 이어짐`, `초기 도입은 A안이 적절함`, `무료 유저 수익화는 필요하나 총매출 임팩트는 제한적`처럼 판단이 바로 보이는 문장을 쓰는 것이 좋습니다.

이 기준을 그대로 요청문에 넣고 싶다면 아래처럼 말하면 됩니다.

```text
conjira를 사용해서 이 문서를 Confluence에 올려줘. 원문 내용은 유지하되, 장황한 설명은 줄이고 메모형 문체로 정리해줘. 핵심 요약을 앞에 두고, 제목 3/4와 표를 활용해서 스캔하기 쉽게 만들어줘. 불릿은 병렬 정보에만 쓰고, 추상적인 수식어는 줄여줘.
```
