...
```text
conjira를 사용해서 이 문서를 Confluence에 올려줘. 원문 내용은 유지하되, 장황한 설명은 줄이고 보고용 메모체로 정리해줘. `~입니다/~합니다`체는 최소화하고, 명사형이나 짧은 판단형 중심으로 써줘. 핵심 요약을 앞에 두고, 제목 3/4와 표를 활용해서 스캔하기 쉽게 만들어줘. 불릿은 병렬 정보에만 쓰고, 추상적인 수식어는 줄여줘.
```
