...
def _parse_blockquote(lines: list[str], start: int) -> tuple[str, int]:
    ...
    if quote_lines:
        ...
        if callout_match is not None:
            ...
            body_html = "".join(body_parts).strip() or "<p></p>"
            return _render_callout_macro(macro_name=macro_name, title=title, body_html=body_html), i

    inner_parts, _ = _parse_blocks(quote_lines, 0)
    return "<blockquote>{0}</blockquote>".format("".join(inner_parts)), i


def _render_callout_macro(*, macro_name: str, title: str, body_html: str) -> str:
    title_xml = ""
    if title:
        title_xml = '<ac:parameter ac:name="title">{0}</ac:parameter>'.format(
            html.escape(title, quote=False)
        )
    return (
        '<ac:structured-macro ac:name="{0}" ac:schema-version="1">'
        "{1}"
        "<ac:rich-text-body>{2}</ac:rich-text-body>"
        "</ac:structured-macro>"
    ).format(
        html.escape(macro_name, quote=True),
        title_xml,
        body_html,
    )
